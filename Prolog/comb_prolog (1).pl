% находит максимальное из 2-х чисел.
max(X,Y,X):-X>Y,!.
max(_,Y,Y).
%находит максимальное из 3-х чисел.
max3(X,Y,U,X):-X>Y,X>U,!.
max3(_,Y,U,Y):-Y>U,!.
max3(_,_,U,U).
%находит факториал числа рекурсией вверх.
fact(0,1):-!.
fact(N,X):-N1 is N-1,fact(N1,X1),X is N*X1.
%находит факториал числа рекурсией вниз.
...
%находит элемент списка, у которого наибольшая сумма цифр.
...
%Этот предикат вычисляет сумму всех элементов в списке рекурсией вверх.
sum_el([], 0) :- !.
sum_el([H|T],Sum) :- sum_el(T, SumT), Sum is H + SumT.
%Этот предикат вычисляет сумму всех элементов в списке рекурсией вниз.
sum_el_down(List, Sum) :- sum_el_down(List, 0, Sum).
sum_el_down([], Sum, Sum) :- !.
sum_el_down([H|T],CurSum, Sum) :- NewSum is CurSum + H, 
	sum_el_down(T, NewSum, Sum).
%предикат вычисляет сумму всех простых элементов в списке рекурсией вниз.
...
%Этот предикат конкатенирует два списка.
append1([], L, L):-!.
append1([H|T], L, [H|R]) :- append1(T, L, R).

% предикат, роверяющий является ли число простым
pr(2):-!.
pr(X):-pr1(X,2).
pr1(X,X):-!.
